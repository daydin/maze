...
static int run(int ms)
{
  ...
  if(step%1080 <360) step_max=360;
  else step_max=180;

  if(step==0){
    tmp= step_max;
  }
  if(0==step%step_max && step!=0){
	  tmp+=step_max;
	  printf("tmp:%d\n",tmp);
  }
  ...
  if(step < TRIAL_STEPS) {
    //drive robot
	  if(step==(tmp-1)) fitness+=run_trial();
	  else run_trial();
	  //track epochs: every (360+4*180)=1080 steps is one epoch
	  if(step%1080 == 0){
  	    epoch++;
  	    reset_neural_network();
  	   // printf("casted epoch: %g\n",(double)epoch);
	  }
	  if(0==(step%step_max)){
		reset_position=1; //at the turn of each trial we need to reset robot's position
	  }else reset_position=0;
	  printf("FITNESS IN THIS TRIAL: %g\n",fitness);
	double msg[4] = {fitness, 0.0,(double)epoch,(double)reset_position};
	wb_emitter_send(emitter, (void *)msg, 4*sizeof(double));
	step++;
  }
  else {
    //stop robot
    ...
    double msg[4] = {fitness, 1.0,0.0,0.0};
    wb_emitter_send(emitter, (void *)msg, 4*sizeof(double));
...
double clip_value(double val, double limit){
...
//		if(tmp[j]>maxIRActivation) maxIRActivation=tmp[j];
//		printf("sensor%d maxIR: %g\n", j, maxIRActivation);
	}

	for(j=0;j<NB_PS;j++) {
		//printf("sensor %d reads %g\n",j,wb_distance_sensor_get_value(ps[j]));
		inputs[j]=(temp[2*j]+temp[(2*j)+1])/2.0;
		printf("input %d reads %g\n",j,inputs[j]);
		//get max IR activation
//		if(tmp[j]>maxIRActivation) maxIRActivation=tmp[j];
//		printf("sensor%d maxIR: %g\n", j, maxIRActivation);
	}
	...
	speed[LEFT]  = (int) clip_value(outputs[0],(double)SPEED_RANGE); //scale  outputs between 0 and 1
	speed[RIGHT] = (int) clip_value(outputs[1],(double)SPEED_RANGE);
