...
void rtoi(){
	...
	double res;
	FILE *pF;
	pF=fopen("real2IntGenome.txt","w+");

	for(i=0;i<POP_SIZE;i++){
		for(j=0;j<NB_ONLY_WT;j++){ //this might be an issue
			res= (pop[i][j]-GENE_MIN)*((double)INT_MAX_PN/(GENE_MAX-GENE_MIN));
		...
		for(j=tmp; j<NB_WEIGHTS;j++){
			res= (pop[i][j]-BIAS_MIN)*((double)INT_MAX_PN/(BIAS_MAX-BIAS_MIN));
			pop_int[i][j]=(int) round(res);
...
void popEncoder (void){
	...
	pF=fopen("encodedPop.txt","w+");
	for(i=0; i<POP_SIZE; i++){
		fprintf(pF,"NEW IND %d\n",i);
		fflush(pF);
		for(j=0; j<NB_WEIGHTS; j++){
            ...
            for(k=0;k<BIT_SIZE;k++){
            	fprintf(pF,"%d",(pop_bin[i][(BIT_SIZE*j)+k])?1:0);
            	fflush(pF);
            }
		}
	}
...
void decodeOne(int *sum, int bits, _Bool* binary, int option){
