...
static void reset(void)
{
  ...
  //  printf("Error: node EPUCK not here!!!\n");
...
static int run(int ms)
{
  ...
  if (!finished) return TIME_STEP; //why not if(finished) return whatever?
...
void resetRobotPosition(void)
{
  ...
  int i;
  for (i=0; i<3; i++) robot_initial_position[i] = 0.0;//0.6*(double)rand()/(double)RAND_MAX-0.3;
  //robot_initial_position[2] = 6.28*(double)rand()/(double)RAND_MAX;
  //robot_initial_position[1]=0.0;
  //robot_rot[3]= 6.28*(double)rand()/(double)RAND_MAX; //chg orientation!
  //supervisor_field_set(robot, SUPERVISOR_FIELD_TRANSLATION_X | SUPERVISOR_FIELD_TRANSLATION_Z | SUPERVISOR_FIELD_ROTATION_ANGLE,robot_initial_position);
  wb_supervisor_field_set_sf_vec3f(trans_field, robot_initial_position);
  wb_supervisor_field_set_sf_rotation(rot_field, robot_rot);
